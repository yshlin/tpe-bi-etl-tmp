...
def extract_load_gcs(src, prefix, dst):
    for month in month_list:
        ...
        os.system('iconv -f utf-16 -t utf-8 ./gp/%s%s_country.csv > ./gp/%s%s_utm_tagged_utf8.csv' % (prefix, mo, prefix, mo))
...
extract_load_gcs(GP_SOURCE, 'retained_installers_org.mozilla.firefox.vpn_', '%s/gp/' % GP_DEST)
extract_load_gcs(GP_SOURCE, 'retained_installers_org.mozilla.rocket_', '%s/gp/' % GP_DEST)
